...
def build_director_prompt(rewrite_payload: dict[str, Any]) -> tuple[str, str]:
    system_prompt = (
    ...
    user_prompt = f"""
请根据 rewrite.json 生成导演分镜。

要求：
1. 保持同一个主角、服装、空间、色调连续。
2. 镜头运动要有变化，只能从这些里选择：zoom_in, zoom_out, pan_left_to_right, pan_right_to_left, drift_center。
3. 每个 scene 都要输出静态出图 prompt 和动态视频 prompt。
4. narration 必须与 rewrite 的 spoken_text 一致，方便后续字幕和配音对齐。
5. transition 只允许 cut、dissolve、bridge。
6. 返回 JSON，结构必须是：
{{
  "title": "...",
  "style": "...",
...
def build_scene_plan(director_payload: dict[str, Any], rewrite_payload: dict[str, Any], duration_map: dict[int, int]) -> dict[str, Any]:
        ...
        scenes.append(
            {
                ...
                "continuity_anchor": str(scene.get("continuity_anchor") or beat.get("continuity_note") or "").strip(),
                "camera_move": str(scene.get("camera_move") or "drift_center").strip(),
                "transition": str(scene.get("transition") or "dissolve").strip(),
                "image_prompt": str(scene.get("image_prompt") or "").strip(),
                "video_prompt": str(scene.get("video_prompt") or "").strip(),
...
def command_run(args: argparse.Namespace, config: dict[str, Any]) -> int:
    ...
    director_system, director_user = build_director_prompt(rewrite_payload)
    ...
    scene_plan = build_scene_plan(director_payload, rewrite_payload, duration_map)
    write_json(project.storyboard / "scene_plan.json", scene_plan)

    route_a_manifest: dict[str, Any] = {"items": []}
    if not args.skip_route_a:
        route_a_manifest = run_route_a(config, config_path, project, project.storyboard / "scene_plan.json")

    if not args.skip_route_b:
        if args.skip_route_a:
            raise StoryProjectError("Route B currently depends on route A images. Do not skip route A when running route B.")
        run_route_b(config, project, scene_plan, route_a_manifest)

    final_manifest = build_final_outputs(config, project)
    write_compare_report(project, scene_plan, not args.skip_route_a, not args.skip_route_b, final_manifest)
